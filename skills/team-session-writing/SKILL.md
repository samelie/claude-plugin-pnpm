...
Bare ids (`R3`, `PD-4`, `AC-2`, `Q-7`, `P-1`, `UREQ-1`, `T-5`) are machine handles — unreadable to a human at a decision point. Rule:
...
- `P-*`: registered `map.md` Premises only. A premise is a falsifiable assumption supporting a recorded decision.
- `UREQ-*`: user propositions and requested outcomes. Never use `P-*` for these.
- Source facts and lead methods carry explicit kind fields. Do not relabel either as a user proposition.
