...
Designer returns approaches and writes `designer/explore.md`. Present one self-contained decision card.
Use the eligible native question tool, remote HITL protocol, or equivalent inline card; transport fields do
...
|--------|-----------------|---------------|
| A: {name} | {concrete output} | {main tradeoff} |
| B: {name} | {concrete output} | {main tradeoff} |
| C: {name} | {concrete output} | {main tradeoff} |

Recommended: {option} — {reason}
Question: Which option best fits the stated scope?
```

Record the user's selected proposition with a `UREQ-*` id and exact user turn. Record an implementation
choice within existing authorization as a method. A material commitment outside existing authorization
remains unratified until explicitly covered; see [approval.md](approval.md).
