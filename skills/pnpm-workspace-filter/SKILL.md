...
- Before selecting test files or names, read the package's `test` script and underlying runner's argument syntax. Do not assume a `--` separator forwards a filter. Record the exact command and actual suites/tests/skips; a broad passing run is not proof that targeting worked.
