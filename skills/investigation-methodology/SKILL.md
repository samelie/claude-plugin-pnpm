...
- **Prefer the CLI**: `ccc search "<concept>"` — works in every agent (Bash), can't go stale against the daemon
- Run 4-5 queries covering different aspects of the investigation
- Useful flags: `--path` (glob, e.g. `"src/utils/*"`), `--lang` (e.g. `typescript`), `--limit`, `--offset` (paginate), `--json` (structured)
- `mcp__cocoindex-code__search` is the same search with args `paths`/`languages`/`limit`/`offset`, but reaches only the DEFAULT agent and breaks after a mid-session `ccc` upgrade — see `.claude/docs/third-party/cocoindex-code.md`
