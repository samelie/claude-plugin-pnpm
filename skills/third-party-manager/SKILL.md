...
Manage Claude Code plugin dependencies: context-mode, caveman, cocoindex-code.
...
echo -e "\n--- cocoindex-code ---"
uv tool list 2>/dev/null | grep cocoindex-code || echo "NOT INSTALLED"
...
ccc search "a concept you know exists" | head -5   # doctor alone does NOT prove search works
echo "Done."
```

⚠️ Upgrading mid-session leaves the `ccc mcp` server process on the OLD client → every MCP query
returns zero results (`Daemon version mismatch`). `ccc daemon restart` does not fix it. Reconnect
with `/mcp`, or restart the session. The CLI path is unaffected — prefer it.
...
case $TOOL in
  ...
  *)
    ...
    echo "Options: context-mode, caveman, cocoindex-code"
    ;;
esac
```
