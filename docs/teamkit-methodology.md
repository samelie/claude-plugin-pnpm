...
| # | Rule | Evidence |
|---|------|----------|
| Bridge | `agent({agentType:'team-coder'})` loads the role agent verbatim (bare names post plugin→`.claude/` migration); composes with `schema` | spike 1; doc-confirmed 2026-08-03 |
| 2 | Custom agentType in the workflow sandbox = frontmatter-derived. ~~Zero `mcp__*`/ToolSearch/Glob/Grep~~ **corrected 2026-09-04**: frontmatter `mcp__*` globs project (2026-08-03), and a frontmatter `ToolSearch`/`Glob` grant ALSO arrives in-lane — the old probes only tested roles whose frontmatter never requested them (confound) | probed 5 types; re-probed codegraph-eval `research-findings-r7-q7-verification.md` |
| 3 | ~~Only the DEFAULT agent reaches MCP~~ **corrected 2026-09-04**: any agent reaches MCP via frontmatter globs; with `ToolSearch` also in frontmatter the globs defer (cheap) in BOTH lanes | probe + 2026-09-04 re-probe |
...
| 16 | Subagent Write guard denies `findings*`/`report*` basenames (BOTH lanes, filename-prefix-triggered; Bash unintercepted). Researcher artifacts renamed `research-findings*.md` 2026-08-04 (probed writable); write-denial protocol (`team-session-writing`) is the standing backstop | probes 2026-08-03/04 (`wf_f161857a-342`, `wf_24219667-1c1`, native, `wf_b3666818-85c`) |

Knowledge routing (**path A**): knowledge stages run as the DEFAULT agent (ToolSearch→MCP) with the role injected; execution stages use custom `agentType`. The `context-mode` Skill wrapper CANNOT bridge to MCP inside a custom agent — Bash/ripgrep survive there, and `ccc` (pure Bash CLI) WORKS (corrected 2026-08-01; the old blanket claim wrongly included it). NOTE 2026-08-03 (live probe `wf_f6675267-c10`): frontmatter `mcp__*` globs PROJECT into custom workflow agents (direct MCP calls succeed live). NOTE 2026-09-04 (codegraph-eval): the "custom types lack ToolSearch/Glob/Grep" half is REFUTED — those probes were confounded (no probed frontmatter ever requested ToolSearch). A frontmatter `ToolSearch` grant works in-lane, defers the MCP globs, and completes fetch→call; the 7 research-side roles now carry it. See SKILL rules 2/3.
