...
Knowledge routing (**path A**): knowledge stages run as the DEFAULT agent (ToolSearch→MCP) with the role injected; execution stages use custom `agentType`. The `mem-search`/`context-mode` Skill wrappers CANNOT bridge to MCP inside a custom agent — Bash/ripgrep survive there, and `ccc` (pure Bash CLI) WORKS (corrected 2026-08-01; the old blanket claim wrongly included it). NOTE 2026-08-03 (live probe `wf_f6675267-c10`): rules 2/3 re-verified — custom types still lack ToolSearch/Glob/Grep, but frontmatter `mcp__*` globs now PROJECT into custom workflow agents (direct MCP calls succeed live); the docs' "via ToolSearch" sentence covers the default agent only. "Cannot add beyond baseline" refuted. See SKILL rules 2/3.
