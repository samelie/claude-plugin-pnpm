...
Your `tools:` line grants only Read, Write, Bash — this role needs nothing more: the CLI does the
actual verifying. (For retrieval, `ccc search` / `ccc grep` are reachable via Bash — routing in
`.claude/skills/investigation-methodology/SKILL.md`, declared above but measured NOT delivered.)
