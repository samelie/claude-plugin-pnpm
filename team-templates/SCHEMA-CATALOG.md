...
| Model | Mechanism | Use |
|-------|-----------|-----|
| FILE (legacy/team-style) | agent writes `{session}/{agent}/file.md`, peer reads via read-findings | bulk artifacts a downstream agent reads in full (design.md, research-findings.md, large diffs) |
...
Default-agent (path A) research/investigation output. Maps to `researcher/research-findings.md`.

```js
const ResearchFindings = {
  topic: 'string',
  summary: 'string',                                  // research-findings.md "Summary"
  keyFindings: [{ claim, evidence, file, lines, snippet }],  // "Key Findings" + file paths
  patterns: ['string'],                               // "Patterns Discovered"
  recommendations: ['string'],
  openQuestions: ['string'],
  sessionFile: 'string',                              // researcher/research-findings.md (bulk)
...
**Axis-B fidelity check for the committed spine — optional by default, MANDATORY when the plan declares the evidence/applicability protocol** (authority for when, what must project and renewal: `skills/team-kit-run/SKILL.md` entry-mode-1 step 3; this section owns only the shape). `team-spec-reviewer` reads the md ground-truth (`team-plan.md` + `design.md`) AND the authored `plan.workflow.js`, then judges whether the script covers the plan — every phase, role, ownership lane, task, AC present; nothing invented; nothing dropped. The script is the "implementation", the md is the "spec" → a spec-compliance review of the authored workflow. `verdict != approved` → revise the script with `missing`/`invented` fed back. Companion to the Axis-A advisory lint (`scripts/validate-workflow.mjs`). By default, run it for high-stakes plans and skip it for small/obvious ones — don't over-ceremony. Under the protocol it is never skipped: it lands before dependent execution, and the verdict is recorded at its RESERVED projection file (e.g. `spec-reviewer/projection-{n}.md`; the `build-state.md` reservation is decisive), which is what `sessionFile` carries — the fixed `alignment-review.md` is then at most a latest pointer, never the decisive record. Field list unchanged.
...
const AlignmentVerdict = {
  ...
  notes: 'string',
  sessionFile: 'string',                              // alignment-review.md · protocol: reserved spec-reviewer/projection-{n}.md
  status: 'clean|errors_remaining', errorCount: 0,
}
```
...
|--------|----------------|
| ResearchFindings | `researcher/research-findings.md` |
...
| AlignmentVerdict | `alignment-review.md` (session root; Axis-B fidelity check, optional by default) · under the evidence protocol: the reserved projection file, e.g. `spec-reviewer/projection-{n}.md` (`alignment-review.md` then at most a latest pointer) |

Drift resolved (0.3.0): agent docs aligned to the canonical paths above — `team-reviewer` writes `reviewer/review-{task-id}.md` (was `research-findings.md`), `team-spec-reviewer` writes to the `spec-reviewer/` subdir (was session root), `team-security-auditor` writes `security-audit.md` (was `report.md`), and SESSION-SCHEMA verifier output is `verifier/results.md` (was `verification.md`).
